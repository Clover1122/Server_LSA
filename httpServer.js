...
// adapted from Claire week week5 practical-server.js
...
app.get('/getPOI', function (req,res) {
	pool.connect(function(err,client,done) {
		...
		// sourced from http://www.postgresonline.com/journal/archives/267-Creating-GeoJSON-FeatureCollections-with-JSON-and-PostGIS-functions.html,accessed 4th January 2018
...
app.post('/uploadData',function(req,res){
	...
	pool.connect(function(err,client,done) {
		...
		querystring = querystring + req.body.question_id + req.body.question + "','" + req.body.answer_1 + "','";
		querystring = querystring + req.body.answer_2 + "','" + req.body.answer_3 + "','" + req.body.answer_4 + "','" + req.body.correct_answer +"',"+geometrystring + "))";
		console.log(querystring);
		client.query( querystring,function(err,result) {
		done();
		if(err){
...
app.post('/uploadAnswer', function(req,res){
	...
	pool.connect(function(err,client,done) {
		...
		var querystring = "INSERT into useranswer (question_id,question,answer,correct_answer) values ('";
		...
		client.query(querystring,function(err,result) {
			...
			res.status(200).send("Answer Chosen Has Been Submitted Successfully");
		});
	});
});
